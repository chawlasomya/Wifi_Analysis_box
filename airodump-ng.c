...
#define MAXIMUM_TIMESTAMP_LENGTH    80
...
int dump_initialize( char *prefix, int ivs_only )
{
	...
	if (G.output_format_protobuf) {
		memset(ofn, 0, ofn_len);
		snprintf( ofn,  ofn_len, "%s-%02d.%s",
				  prefix, G.f_index, AIRODUMP_NG_PROTOBUF_EXT );
...
char* sha_encoding(char* idString, unsigned char* outputStr, char* finalStr)
{
    char* final_encode;
    int loopCntr = 0;
    SHA512_CTX ctx;
    SHA_CTX ctx2;
    SHA512_Init(&ctx);
    SHA512_Update(&ctx, idString, strlen(idString));
    SHA512_Final(outputStr, &ctx);
    for (loopCntr = 0; loopCntr < SHA512_DIGEST_LENGTH; loopCntr++) {
        sprintf(&finalStr[loopCntr * 2], "%02x",
                (unsigned char) outputStr[loopCntr]);
    }
    outputStr[0] = '\0';
    SHA1_Init(&ctx2);
    SHA1_Update(&ctx2, finalStr, strlen(finalStr));
    SHA1_Final(outputStr, &ctx2);
    for (loopCntr = 0; loopCntr < SHA_DIGEST_LENGTH; loopCntr++) {
        sprintf(&finalStr[loopCntr * 2], "%02x",
                (unsigned char) outputStr[loopCntr]);
    }
    finalStr[SHA_DIGEST_LENGTH * 2] = '\0';
    fprintf(stdout,"test\n");
    final_encode = malloc(sizeof(finalStr)+1);
    //memcpy(final_encode,finalStr,(sizeof(finalStr)));
    fprintf(stdout,"%p\n",final_encode);
    return(final_encode);
}

int dump_write_csv_or_protobuf(enum OutputType outputType)
{
    ...
    if (OUTPUT_TYPE_CSV == outputType) {
        ...
        fseek(G.f_protobuf, 0, SEEK_SET);
        /* Once we commit all the changes to the code will modify wb+ to w */
        /* Retaining the CSV in /tmp for debugging */
    ...
    if( OUTPUT_TYPE_PROTOBUF == outputType ) {
        ...
        ltime = localtime(&now);
        beacon.timestamp = (char*) malloc(MAXIMUM_TIMESTAMP_LENGTH);
    ...
    while (ap_cur != NULL) {
        ...
        sprintf(idString, "%02X:%02X:%02X:%02X:%02X:%02X:%02X",
                ap_cur->bssid[0], ap_cur->bssid[1], ap_cur->bssid[2],
                ap_cur->power_bit, ap_cur->bssid[3], ap_cur->bssid[4],
                ap_cur->bssid[5]);
        sha = sha_encoding(idString, outputStr, finalStr);
        /*SHA512_Init(&ctx);
        SHA512_Update(&ctx, idString, strlen(idString));
        SHA512_Final(outputStr, &ctx);
        for (loopCntr = 0; loopCntr < SHA512_DIGEST_LENGTH; loopCntr++) {
        ...
        fprintf(stdout,"%s\n",sha);
        fflush(stdout);
        fprintf(G.f_txt, "%s, ", sha);
        if (OUTPUT_TYPE_PROTOBUF == outputType) {
            beacon.bssids[ap_id]->bssid = sha;
            //memcpy(beacon.bssids[ap_id]->bssid, finalStr, sizeof(*sha));
        }
        //free(sha);
        ...
        if (OUTPUT_TYPE_PROTOBUF == outputType) {
            ...
            strftime(beacon.bssids[ap_id]->firsttimeseen, MAXIMUM_TIMESTAMP_LENGTH, "%FT%TZ", ltime);
        }
        ...
        if( OUTPUT_TYPE_PROTOBUF == outputType ) {
            ...
            memcpy(beacon.bssids[ap_id]->privacy, print_buffer, (strlen(print_buffer)+1));
        }
        ...
        if(OUTPUT_TYPE_PROTOBUF == outputType) {
            ...
            memcpy(beacon.bssids[ap_id]->cipher, print_buffer, (strlen(print_buffer)+1));
        }
        ...
        if( OUTPUT_TYPE_PROTOBUF == outputType ) {
            ...
            memcpy(beacon.bssids[ap_id]->authenication, print_buffer, (strlen(print_buffer)+1));
        }

        fprintf(G.f_txt, ", %3d, %8ld, %8ld, ",
                ap_cur->avg_power, ap_cur->nb_bcn, ap_cur->nb_data);
        if (OUTPUT_TYPE_PROTOBUF == outputType) {
        ...
        memset(print_buffer, 0, sizeof(print_buffer));
        if (OUTPUT_TYPE_PROTOBUF == outputType) {
            sprintf(print_buffer,"%3d.%3d.%3d.%3d, ", ap_cur->lanip[0], ap_cur->lanip[1], ap_cur->lanip[2], ap_cur->lanip[3]);
            beacon.bssids[ap_id]->lanip = (char* ) malloc(strlen(print_buffer)+1);
            memcpy(beacon.bssids[ap_id]->lanip,print_buffer, (strlen(print_buffer)+1));
        }
        ...
        if(OUTPUT_TYPE_PROTOBUF == outputType) {
            ...
            memcpy(beacon.bssids[ap_id]->essid, temp, ((strlen(temp))+1));
        }
        free(temp);
    ...
    while (st_cur != NULL) {
        ...
        sprintf(idString, "%02X:%02X:%02X:%02X:%02X:%02X:%02X",
                ...
                ap_cur->power_bit, st_cur->stmac[3], st_cur->stmac[4],
                st_cur->stmac[5]);
        SHA512_Init(&ctx);
        SHA512_Update(&ctx, idString, strlen(idString));
        SHA512_Final(outputStr, &ctx);
        ...
        if (OUTPUT_TYPE_PROTOBUF == outputType) {
            beacon.devices[st_id]->mac = (char*) malloc(sizeof(finalStr));
            memcpy(beacon.devices[st_id]->mac, finalStr, sizeof(finalStr));
        }
        ...
        if (OUTPUT_TYPE_PROTOBUF == outputType) {
            ...
            strftime(beacon.devices[st_id]->firsttimeseen, MAXIMUM_TIMESTAMP_LENGTH, "%FT%TZ", ltime);
        }
        ...
        if ( OUTPUT_TYPE_PROTOBUF == outputType ) {
            ...
            strftime(beacon.devices[st_id]->lasttimeseen, MAXIMUM_TIMESTAMP_LENGTH, "%FT%TZ", ltime);
        }
        ...
        if (!memcmp(ap_cur->bssid, BROADCAST, 6)) {
            ...
            if (OUTPUT_TYPE_PROTOBUF == outputType) {
                ...
                memcpy(beacon.devices[st_id]->bssid, print_buffer, ((strlen(print_buffer))+1));
            }
        }
        else {
            idString[0] = '\0';
            ...
            fprintf(G.f_txt, "%s, ", finalStr);
            if (OUTPUT_TYPE_PROTOBUF == outputType) {
                beacon.devices[st_id]->bssid = (char*) malloc(sizeof(finalStr)+1);
                memcpy(beacon.devices[st_id]->bssid, finalStr, sizeof(finalStr));
            }
        }
        ...
        for (i = 0, n = 0; i < NB_PRB; i++) {
            ...
            fprintf(G.f_txt, "%s", finalStr);
            if( OUTPUT_TYPE_PROTOBUF == outputType) {
                beacon.devices[st_id]->probedessid[n] = (char*) malloc(sizeof(finalStr));
                memcpy(beacon.devices[st_id]->probedessid[n], finalStr, sizeof(finalStr));
                n++;
            }
            free(temp);
        }
        ...
        if( OUTPUT_TYPE_PROTOBUF == outputType ) {
            if (st_cur->vendor_index !=0) {
               beacon.devices[st_id]->vendorouid = (char**) malloc (sizeof(char*) * (st_cur->vendor_index));
            }
        ...
        for (loopCntr = 0; loopCntr < st_cur->vendor_index; loopCntr++) {
            ...
            memset(finalStr, 0, sizeof(finalStr) - 1);
            for (i = 0; i < 3; i++) {
                sprintf(&finalStr[i * 2], "%02x",
                        (unsigned char) st_cur->vendor[loopCntr][i]);
            }
            fprintf(G.f_txt, "%s,", finalStr);
            if( OUTPUT_TYPE_PROTOBUF == outputType ) {
                beacon.devices[st_id]->vendorouid[loopCntr] = (char*) malloc(sizeof(finalStr));
                memcpy(beacon.devices[st_id]->vendorouid[loopCntr], finalStr, sizeof(finalStr));
            }
        }
...
//             printf("finished rearranging\n");
    ...
    while( 1 )
    {
        ...
        if( time( NULL ) - tt1 >= G.file_write_interval )
        {
            ...
            tt1 = time( NULL );
            if (G. output_format_csv)  dump_write_csv_or_protobuf(OUTPUT_TYPE_CSV);
            if (G.output_format_protobuf) dump_write_csv_or_protobuf(OUTPUT_TYPE_PROTOBUF);
